...
@section('content')
    <div class="row">
        <div class="col-12 col-md-4 offset-md-4">
            <form action="{{ route('masterdata.ongkir.update', ["ongkir" => $ongkir->id ])}}" method="post" id="myForm">
                @csrf
                @method('PATCH')
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row mb-5">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('ongkir') is-invalid @enderror" name="ongkir" value="{{ isset($ongkir->ongkir) ? $ongkir->ongkir :old('ongkir')}}" placeholder="Ongkir Per Kg">
                                    @error('ongkir')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
...
<script>
    jQuery(document).ready(function(){
        ...
        $('select[name="shipping_id"').val("{{ isset($ongkir->shipping_id) ? $ongkir->shipping_id : old('shipping_id') }}").trigger('change')
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('select[name="province_id"]').val("{{ isset($ongkir->province_id) ? $ongkir->province_id : old('province_id') }}").trigger('change')
            },
        });
        ...
        $('select[name="province_id"]').on('change', function(){
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('select[name="city_id"]').val("{{ isset($ongkir->city_id) ? $ongkir->city_id : old('city_id') }}").trigger('change')
                },
            });
        });
